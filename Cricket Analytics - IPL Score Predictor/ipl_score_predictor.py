...
batting_team= st.selectbox('Select the Batting Team ',('Royal Challengers Bangalore','Chennai Super Kings', 'Delhi Daredevils', 'Kings XI Punjab','Kolkata Knight Riders','Mumbai Indians','Rajasthan Royals','Sunrisers Hyderabad'))
#SELECT BOWLING TEAM
...
if st.button('Predict Score'):
    if bowling_team==batting_team:
        st.success("Batting and Bowling Teams Should Be Different")
    elif overs-math.floor(overs)>0.5:
        st.success("Please enter valid over input as one over only contains 6 balls")  
    else:
        #Call the ML Model
